...
class Limiter
{
    ...
    public static function getClass($max_request = 100, $period = 60, $name = 'limiter', $config = 'cache.limiter')
    {
        $set = [$max_request, $period, $name, $config];
        $key = md5(serialize($set));
        if (!isset(self::$instance[$key])) {
            self::$instance[$key] = new Limiter($max_request, $period, $name, $config);
        }

        return self::$instance[$key];
    }

    public function __construct($max_request, $period, $name, $config)
    {
        $this->name = $name;
